...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'claudio',
    launchCommand: 'claudio --project-aware',
    terminalName: 'Claudio',
    shimEnabled: false,
    workspaceFolder: '/workspace/claudio/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/claudio/very/long/path/example-project',
    launchCwdLabel: '/workspace/claudio/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/claudio/very/long/path/example-project/.claudio-profile.json',
    workspaceProfilePath: '/workspace/claudio/very/long/path/example-project/.claudio-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Claudio wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(html, /Claud<span class="wordmark-accent">io<\/span>/);
  ...
  assert.match(
    html,
    /title="\/workspace\/claudio\/very\/long\/path\/example-project"[^>]*>\/workspace\/claudio\/very\/long\/path\/example-project<\//,
  );
});
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/claudio/src/panels/control-center.js',
      workspacePath: '/workspace/claudio',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/claudio/src/panels',
      projectAwareCwdLabel: '/workspace/claudio/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/claudio',
      workspaceRootCwdLabel: '/workspace/claudio',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/claudio',
      activeFilePath: '/workspace/claudio/src/panels/control-center.js',
      workspacePath: '/workspace/claudio',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/claudio',
      projectAwareCwdLabel: '/workspace/claudio',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/claudio/src/panels',
      launchCwdLabel: '/workspace/claudio/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/claudio\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/claudio/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/claudio',
      launchCwdLabel: '/workspace/claudio',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/claudio/);
  assert.match(html, /Same workspace-root target as Launch Claudio because the relative command resolves from the workspace root · \/workspace\/claudio/);
});
