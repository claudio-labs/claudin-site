...
test('resolveCommandCheckPath resolves workspace-relative executables', () => {
  assert.equal(
    resolveCommandCheckPath('./node_modules/.bin/claudio', '/repo'),
    require('node:path').resolve('/repo', './node_modules/.bin/claudio'),
  );
});

test('resolveCommandCheckPath leaves bare commands alone', () => {
  assert.equal(resolveCommandCheckPath('claudio', '/repo'), null);
});

test('findCommandPath treats shell-like input as a literal executable name', t => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'claudio-command-'));
  t.after(() => {
