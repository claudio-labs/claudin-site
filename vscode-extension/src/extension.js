...
const PROFILE_FILE_NAME = '.claudio-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('claudio');
  const launchCommand = configured.get('launchCommand', 'claudio');
...
async function launchClaudio(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Claudio command not found: ${executable}. Install it with: npm install -g @gitlawb/claudio`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await openExternalUrl(CLAUDIO_SETUP_URL);
    } else if (action === 'Open Repository') {
      await openExternalUrl(CLAUDIO_REPO_URL);
    }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Claudio wordmark">Claud<span class="wordmark-accent">io</span></div>
            <div class="headline">
...
function activate(context) {
  ...
  statusBarItem.command = 'claudio.openChat';
  statusBarItem.show();
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('claudio.start', async () => {
    await launchClaudio();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'claudio.startInWorkspaceRoot',
    async () => {
      await launchClaudio({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('claudio.openDocs', async () => {
    await openExternalUrl(CLAUDIO_REPO_URL);
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'claudio.openSetupDocs',
    async () => {
      await openExternalUrl(CLAUDIO_SETUP_URL);
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'claudio.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('claudio.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.claudio');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('claudio.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('claudio.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('claudio.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('claudio.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'claudio.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('claudio')) {
        refreshProvider();
      }
    }),
