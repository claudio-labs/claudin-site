...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/claudio/workspace/.claudio-profile.json', 30),
    '.../.claudio-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\claudio\\workspace\\.claudio-profile.json', 30),
    '...\\.claudio-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\claudio\\workspace\\.claudio-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.claudio-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/claudio',
    ...
    profileStatusHint: '/workspace/claudio/.claudio-profile.json',
    workspaceProfilePath: '/workspace/claudio/.claudio-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'claudio',
          detail: '/workspace/claudio · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/claudio/.claudio-profile.json',
          tone: 'neutral',
        },
      ],
    },
